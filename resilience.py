...
class CellFailover:
    ...
    def find_failover_target(self, failed_coord: HexCoord) -> HexCoord | None:
        """
        ...
        for neighbor in failed_cell.get_all_neighbors():
            if (
                neighbor
                and isinstance(neighbor, WorkerCell)
                and neighbor.coord not in self._failed_cells
                and neighbor.is_available
                and neighbor.load < self.config.degraded_load_threshold
            ):
        ...
        for coord in failed_coord.ring(radius):
            ...
            if (
                cell
                and isinstance(cell, WorkerCell)
                and coord not in self._failed_cells
                and cell.is_available
                and cell.load < self.config.degraded_load_threshold
            ):
                return coord
...
class Vote:
    ...
    def sign(self, key: bytes | None = None) -> Vote:
...
class QueenSuccession:
    ...
    def elect_new_queen(self) -> QueenCell | None:
        """
    ...
    def _pick_best_candidate(
        ...
        """
        best_candidate: HexCoord | None = None
    ...
    def _tally_votes(
        ...
        for vote in votes:
            if not vote.verify():
                ...
                logger.warning("Vote from %s rejected: invalid/missing signature", vote.voter)
                continue
    ...
    def _conduct_election(self, candidates: list[HexCoord]) -> HexCoord | None:
        """
    ...
    def _promote_to_queen(self, coord: HexCoord) -> QueenCell | None:
        ...
        # Registrar workers
        for _c, cell in self.grid._cells.items():
...
class HexRedundancy:
    ...
    def __init__(
        ...
        strategy: HexRedundancy.Strategy | None = None,
    ):
        self.grid = grid
        self.config = config
    ...
    def _setup_mirror_replication(self, coord: HexCoord, cell: HoneycombCell) -> list[HexCoord]:
    ...
    def _setup_ring_replication(self, coord: HexCoord, cell: HoneycombCell) -> list[HexCoord]:
    ...
    def _setup_quorum_replication(self, coord: HexCoord, cell: HoneycombCell) -> list[HexCoord]:
    ...
    def read_with_fallback(self, coord: HexCoord) -> dict[str, Any] | None:
        """
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            stale_mirrors = sum(
                1 for mirrors in self._mirrors.values() for m in mirrors if m.is_stale()
            )
...
class SwarmRecovery:
    ...
    def assess_damage(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            for coord, cell in self.grid._cells.items():
                if cell.state == CellState.FAILED:
                    ...
                    report["affected_rings"].add(coord.distance_to(HexCoord.origin()))
        ...
        report["damage_percentage"] = report["failed_cells"] / report["total_cells"] * 100
    ...
    def create_recovery_plan(self) -> list[tuple[HexCoord, RecoveryAction]]:
        ...
        with self._lock:
            ...
            damaged_by_distance = sorted(
                self._damaged_cells, key=lambda c: c.distance_to(HexCoord.origin())
            )
    ...
    def execute_recovery_plan(
        self, plan: list[tuple[HexCoord, RecoveryAction]] | None = None
    ) -> dict[str, int]:
        """
    ...
    def _execute_single_recovery(self, coord: HexCoord, action: RecoveryAction) -> bool:
    ...
    def _rebuild_cell(self, coord: HexCoord, cell: HoneycombCell) -> bool:
        ...
        # Limpiar estado.
        # B11 (Phase 3): el atributo correcto es ``_pheromone_field`` (una
        # ``PheromoneField``), NO ``_pheromone_level`` (que no existe). El
        # código previo creaba silenciosamente un atributo muerto y dejaba
        # la feromona original intacta tras el rebuild. Reemplazamos el
        # field entero por uno nuevo para resetear todas las intensidades.
        cell._vcores.clear()
        cell._error_count = 0
        cell._pheromone_field = PheromoneField()  # type: ignore[no-untyped-call]
...
class CombRepair:
    ...
    def __init__(self, grid: HoneycombGrid):
        self.grid = grid
        self._repairs_performed: list[dict[str, Any]] = []
    ...
    def _check_neighbor_connections(
        self, coord: HexCoord, cell: HoneycombCell
    ...
    def _check_data_integrity(
        ...
        try:
            ...
            issues.append(
                self.RepairIssue(coord=coord, issue_type="corrupt_metadata", severity=8, details={})
            )
    ...
    def repair_all(self, issues: list[CombRepair.RepairIssue] | None = None) -> dict[str, int]:
        """
...
class HiveResilience:
    ...
    def __init__(self, grid: HoneycombGrid, config: ResilienceConfig | None = None):
        self.grid = grid
    ...
    def tick(self) -> dict[str, Any]:
        ...
        """
        self._tick_count += 1
        results: dict[str, Any] = {"tick": self._tick_count}
        ...
        # Verificar reina
        if (
            self._tick_count % self.config.queen_heartbeat_interval == 0
            and not self._succession.check_queen_health()
        ):
            ...
            if report.status == HealthStatus.HEALTHY:
                ...
                if self.config.auto_recovery:
                    self.handle_cell_failure(
                        coord, report.failure_type or FailureType.ERROR_THRESHOLD
                    )
    ...
    def _check_cell_health(self, coord: HexCoord, cell: HoneycombCell) -> HealthReport:
    ...
    def handle_cell_failure(self, coord: HexCoord, failure_type: FailureType) -> FailoverEvent:
        """Maneja un fallo de celda."""
        return self._failover.handle_failure(coord, failure_type)

    def initiate_queen_succession(self) -> QueenCell | None:
